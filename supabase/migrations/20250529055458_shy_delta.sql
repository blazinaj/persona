/*
  ...
  1. Changes
    - Add get_profile_stats function to accurately calculate profile statistics
    - Fix update_profile_stats_trigger function to properly update stats
    - Add a proper function to refresh all profile stats
...
-- Create a separate function to manually refresh profile stats
CREATE OR REPLACE FUNCTION refresh_profile_stats(profile_id_input uuid DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  total_favorites integer;
BEGIN
  -- If profile_id is provided, only refresh that profile
  IF profile_id_input IS NOT NULL THEN
    -- Calculate stats for the specific profile
    SELECT 
      COUNT(*),
      COUNT(*) FILTER (WHERE visibility = 'public')
    INTO
  ...
  ELSE
    ...
    LOOP
      PERFORM refresh_profile_stats(profile_record.id);
    END LOOP;
  END IF;
END;
$$;
