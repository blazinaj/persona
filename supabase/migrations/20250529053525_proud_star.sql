/*
  # Fix get_space_messages function to improve user display
  
  1. Changes
    - Drop existing function before redefining it
    - Improve sender name handling to avoid 'Unknown User' fallback
    - Maintain chronological order (ASC) for messages
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN sm.user_id IS NOT NULL THEN COALESCE(pr.display_name, u.email)
      ELSE 'Unknown'
    END as sender_name,
    CASE 
