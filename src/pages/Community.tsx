...
export const Community = () => {
  ...
  const fetchPublicProfiles = async () => {
    try {
      // Get public profiles with their stats
      const { data: profilesData, error: profilesError } = await supabase
        ...
        .select(`
          *,
          ...
          followers:profile_followers!profile_followers_profile_id_fkey(count)
        `)
        .eq('is_public', true);

      if (profilesError) throw profilesError;

      // Transform and set profiles data
  ...
  const sortedProfiles = [...profiles].sort((a, b) => {
    switch (sortBy) {
      case 'popular':
        return (b.stats.total_views + b.stats.total_favorites * 2) - 
               (a.stats.total_views + a.stats.total_favorites * 2);
      case 'newest':
        return new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
      case 'active':
        return b.stats.public_personas - a.stats.public_personas;
      default:
        return 0;
    }
  });
            ...
            >
              <div className="p-6">
                ...
                <div className="mt-6 grid grid-cols-2 gap-4 border-t border-gray-100 pt-4">
                  <div>
                    <p className="text-2xl font-bold text-gray-900">{profile.stats.public_personas}</p>
                    <p className="text-sm text-gray-600">Public Personas</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-gray-900">{profile.followers_count}</p>
